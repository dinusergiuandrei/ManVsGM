...
public class Database implements Serializable {
    ...
    public void computeCache() {
        ...
        this.data.forEach(
                dataSetEntry -> {
                    ...
                    Table table = Table.computeTableFromFen(dataSetEntry.getPositionFenString());

                    if(table!=null) {
                        addEntryInDatabase(table, dataSetEntry.getMove());

                        for (Table nextTable : Table.getAllPossibleNextTables(table)) {
                            addTableToDatabase(nextTable);
                        }
                    }
                }
        );
    ...
    public void addEntryInDatabase(Table table, Move move){
    ...
    private void addTableToDatabase(Table table){
        ...
        if(!this.getFenToFeaturesValuesMap().keySet().contains(fen)) {
